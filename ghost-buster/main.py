    ...
    def updateProb(self, color, x, y):
        ...
        for w in range(0, WIDTH):
            for h in range(0, HEIGHT):
                dist = abs(w-x) + abs(h-y)
                p = bel[h][w]
                # p(z|x) * \bar{bel}(x)
                if(dist == 0):
                    bel[h][w] = p * pDistantZero[idx]
                elif(dist == 1):
                    bel[h][w] = p * pDistantOne[idx]
                elif(dist == 2 or dist == 3):
                    bel[h][w] = p * pDistantTwoThree[idx]
                else:
                    bel[h][w] = p * pDistantAboveThree[idx]

        prob[:] = bel / sum(sum(bel))
        for h in range(0, HEIGHT):
            for w in range(0, WIDTH):
